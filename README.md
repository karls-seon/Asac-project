...
| 팀원 | 담당 영역 | 세부 |
| :--- | :--- | :--- |
| 이종선 | PM · 텍스트 분석 · 모델링 | Gemini 댓글 분류, BERTopic, XGBoost |
| 곽재양 | 데이터 수집 · EDA · 이미지 분석 · 발표 | 댓글 수집, 엣지 밀도 |
| 김주영 | 데이터 수집 · 이미지 분석 · 발표준비 | 펀딩 데이터 수집, 채도 |
| 김태연 | 텍스트 분석 · 이미지 분석 | XLM-RoBERTa 감성 3분류, 명도·명암비 |
| 조은나라 | 이미지 분석 · 모델링 | 이미지 대표 색상 추출, XGBoost |
...
<img src="03_Docs/thumbnail_metrics.png" width="760" alt="썸네일 4개 지표 추출 예시">

<sub>Eleven: Football Manager Board Game 썸네일에서 추출한 4개 지표</sub>

<img src="03_Docs/thumbnail_colors.png" width="760" alt="대표 색상 4종과 감성 키워드">

<sub>대표 색상 4종 → 감성 키워드. 매핑 기준은 IRI Design Institute의 형용사 이미지 스케일.</sub>


## 실행

`01_Src/processed/2.1_comment_gemini.ipynb` 는 Gemini API 키 필요.
API키는 직접 발급 후 같은 경로에 `api_key.txt` 로 저장하고 실행.
